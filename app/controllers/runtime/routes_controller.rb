  ...
  class RoutesController < RestController::ModelController
    ...
    def self.translate_validation_exception(e, attributes)
      if e.errors.on(:routing_api) == [:routing_api_disabled]
        return Errors::ApiError.new_from_details('TcpRoutingDisabled')
      end

      if e.errors.on(:routing_api) == [:uaa_unavailable]
        return Errors::ApiError.new_from_details('UaaUnavailable', 'The UAA service is currently unavailable')
      end

      if e.errors.on(:routing_api) == [:routing_api_unavailable]
        return Errors::ApiError.new_from_details('RoutingApiUnavailable')
      end
      ...
      if e.errors.on(:port) == [:port_taken]
        ...
        return Errors::ApiError.new_from_details('RoutePortTaken', port_taken_error_message)
      end

      if e.errors.on(:port) == [:port_unsupported]
        return Errors::ApiError.new_from_details('RouteInvalid', 'Port is supported for domains of TCP router groups only.')
      end

      if e.errors.on(:port) == [:port_required]
        return Errors::ApiError.new_from_details('RouteInvalid', 'For TCP routes you must specify a port or request a random one.')
      end
      ...
      if e.errors.on(:host) == [:host_and_path_domain_tcp]
        return Errors::ApiError.new_from_details('RouteInvalid', 'Host and path are not supported, as domain belongs to a TCP router group.')
      end
      ...
      if space_errors && space_errors.include?(:total_reserved_route_ports_exceeded)
        return Errors::ApiError.new_from_details('SpaceQuotaTotalReservedRoutePortsExceeded')
      end
