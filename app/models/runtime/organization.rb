class Organization < Sequel::Model
  ...
  one_to_many :service_instances,
              dataset: -> { ServiceInstance.filter(space: spaces) }

  one_to_many :managed_service_instances,
              dataset: -> { ServiceInstance.filter(space: spaces, is_gateway_service: true) }
  ...
  many_to_many(
    ...
    dataset: proc { |r|
      Domain.dataset.where(owning_organization_id: self.id).
  ...
  one_to_many :domains,
              dataset: -> { Domain.shared_or_owned_by(id) },
