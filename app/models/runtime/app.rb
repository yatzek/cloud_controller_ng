...
class App < Sequel::Model
  ...
  one_to_many :events, class: AppEvent
  many_to_one :app, class: 'AppModel', key: :app_guid, primary_key: :guid, without_guid_generation: true
  many_to_one :admin_buildpack, class: Buildpack
  ...
  def custom_buildpacks_enabled?
    !Config.config[:disable_custom_buildpacks]
  end

  def max_app_disk_in_mb
    Config.config[:maximum_app_disk_in_mb]
  end
  ...
  def to_hash(opts={})
    opts[:redact] = if SecurityContext.admin? || space.has_developer?(SecurityContext.current_user)
  ...
  def ports_with_defaults
    ::Diego::Protocol::OpenProcessPorts.new(self).to_a
  end
