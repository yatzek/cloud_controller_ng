...
class ServiceInstance < Sequel::Model
  ...
  def to_hash(opts={})
    if !SecurityContext.admin? && !space.has_developer?(SecurityContext.current_user)
  ...
  def self.managed_organizations_spaces_dataset(managed_organizations_dataset)
    Space.dataset.filter({ organization_id: managed_organizations_dataset.select(:organization_id) })
  end
