class InstallBuildpacks
  ...
  def install(buildpacks)
    ...
    buildpacks.each do |bpack|
      ...
      buildpack_job = Jobs::Runtime::BuildpackInstaller.new(buildpack_name, buildpack_file, buildpack)
      Jobs::Enqueuer.new(buildpack_job, queue: Jobs::LocalQueue.new(config)).enqueue
    end
  end
