...
describe PaidServiceInstancePolicy do
  ...
  let(:basic_service_plan) { ServicePlan.make(free: true) }
  let(:paid_service_plan) { ServicePlan.make(free: false) }
  let(:service_instance) do
    ManagedServiceInstance.make_unsaved space: space, service_plan: @service_plan
  end
  let(:quota) { QuotaDefinition.make non_basic_services_allowed: non_basic_services_allowed }
