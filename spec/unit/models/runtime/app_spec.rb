...
describe App, type: :model do
  ...
  describe 'Creation' do
    ...
    it 'has a default memory' do
      allow(Config.config).to receive(:[]).with(:default_app_memory).and_return(873565)
  ...
  describe 'Validations' do
    ...
    describe 'quota' do
      ...
      context 'app update' do
        def act_as_cf_admin
          allow(SecurityContext).to receive_messages(admin?: true)
          yield
        ensure
          allow(SecurityContext).to receive(:admin?).and_call_original
        end
  ...
  describe '#environment_json' do
    ...
    describe 'env is encrypted' do
      ...
      let(:last_row) { App.dataset.naked.order_by(:id).last }
    ...
    context 'for a started app' do
      ...
      context 'when lazily backfilling default port values' do
        before do
          ...
          stub_const('App::DEFAULT_PORTS', nil)
          app.update(diego: true)
          stub_const('App::DEFAULT_PORTS', default_ports)
        end
