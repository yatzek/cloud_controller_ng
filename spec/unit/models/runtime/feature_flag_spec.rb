...
describe FeatureFlag, type: :model do
  let(:valid_flags) do
    [:user_org_creation, :private_domain_creation, :app_bits_upload,
     ...
     :task_creation, :space_scoped_private_broker_creation,
  ...
  describe '.enabled?' do
    ...
    context 'when logged in as an admin' do
      before do
        ...
        stub_const('FeatureFlag::DEFAULT_FLAGS', { normal: false, blahrgha: false })
        stub_const('FeatureFlag::ADMIN_SKIPPABLE', [:blahrgha])
      end
