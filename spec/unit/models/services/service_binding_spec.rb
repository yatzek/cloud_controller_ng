...
describe ServiceBinding, type: :model do
  ...
  describe '#in_suspended_org?' do
    let(:app) { App.make }
    subject(:service_binding) {  ServiceBinding.new(app: app) }
  ...
  describe '#to_hash' do
    ...
    it 'redacts creds for a space manager' do
      allow(SecurityContext).to receive(:current_user).and_return(manager)
      expect(binding.to_hash['credentials']).to eq({ redacted_message: '[PRIVATE DATA HIDDEN]' })
    end
  end
