...
describe Service, type: :model do
  ...
  describe '#user_visibility_filter' do
    ...
    before do
      ...
      SecurityContext.set(admin_user, { 'scope' => [Roles::CLOUD_CONTROLLER_ADMIN_SCOPE] })
  ...
  describe '#purge' do
    ...
    context 'there is a service instance with state `in progress`' do
      before do
        service_instance.save_with_new_operation({}, state: ManagedServiceInstance::IN_PROGRESS_STRING)
      end
    ...
    context 'when deleting a service instance fails' do
      ...
      before do
        allow_any_instance_of(ServiceInstance).to receive(:destroy).and_raise('Boom')
      end
    ...
    context 'when deleting a service plan fails' do
      ...
      before do
        allow_any_instance_of(ServicePlan).to receive(:destroy).and_raise('Boom')
      end
