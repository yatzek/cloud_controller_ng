...
describe ServiceInstance, type: :model do
  ...
  describe '#create' do
    ...
    describe 'when is_gateway_service is false' do
      it 'returns a UserProvidedServiceInstance' do
        ...
        expect(described_class.find(guid: service_instance.guid).class).to eq(UserProvidedServiceInstance)
      end
    end
    ...
    describe 'when is_gateway_service is true' do
      it 'returns a ManagedServiceInstance' do
        ...
        service_instance = described_class.create(service_instance_attrs)
        expect(described_class.find(guid: service_instance.guid).class).to eq(ManagedServiceInstance)
      end
    end
  ...
  describe '#type' do
    it 'returns the model name for API consumption' do
      managed_instance = ManagedServiceInstance.new
      expect(managed_instance.type).to eq 'managed_service_instance'

      user_provided_instance = UserProvidedServiceInstance.new
  ...
  describe '#as_summary_json' do
    it 'contains name, guid, and binding count' do
      instance = ServiceInstance.make(
        guid: 'ABCDEFG12',
        name: 'Random-Number-Service',
      )
      ServiceBinding.make(service_instance: instance)
  ...
  describe '#in_suspended_org?' do
    let(:space) { Space.make }
    subject(:service_instance) { ServiceInstance.new(space: space) }
  ...
  describe '#to_hash' do
    ...
    it 'redacts creds for a space auditor' do
      allow(SecurityContext).to receive(:current_user).and_return(auditor)
