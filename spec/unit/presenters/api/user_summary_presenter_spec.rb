...
describe UserSummaryPresenter do
  describe '#to_hash' do
    ...
    let(:managed_org) { Organization.make }
    let(:billing_managed_org) { Organization.make }
    let(:audited_org) { Organization.make }
    let(:space) { Space.make(organization: org) }
    let(:managed_space) { Space.make(organization: managed_org) }
    let(:audited_space) { Space.make(organization: audited_org) }
    let(:user) do
