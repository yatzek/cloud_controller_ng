...
module Jobs
  module Services
    describe ServiceInstanceStateFetch do
      ...
      subject(:job) do
        Jobs::Services::ServiceInstanceStateFetch.new(
          name,
          client_attrs,
      ...
      describe '#initialize' do
        ...
        before do
          allow(Config).to receive(:config).and_return({
      ...
      describe '#perform' do
        ...
        context 'when the poll_interval is changed after the job was created' do
          let(:default_polling_interval) { Config.config[:broker_client_default_async_poll_interval_seconds] }
          ...
          before do
            ...
            updated_config = Config.config.merge(
              {
                broker_client_default_async_poll_interval_seconds: new_polling_interval
              })
            allow(Config).to receive(:config).and_return(updated_config)
          end
