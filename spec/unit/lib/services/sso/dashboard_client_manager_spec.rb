...
module VCAP::Services::SSO
  describe DashboardClientManager do
    ...
    let(:services_event_repository) { Repositories::ServiceEventRepository.new(user: user, user_email: email) }
    ...
    context 'for service brokers' do
      ...
      describe '#synchronize_clients_with_catalog' do
        ...
        describe 'modifying the UAA and CCDB' do
          context 'when no dashboard sso clients present in the catalog exist in UAA' do
            ...
            it 'claims the clients' do
              expect {
                manager.synchronize_clients_with_catalog(catalog)
              }.to change { ServiceDashboardClient.count }.by 2

              expect(ServiceDashboardClient.find(uaa_id: dashboard_client_attrs_1['id'])).to_not be_nil
              expect(ServiceDashboardClient.find(uaa_id: dashboard_client_attrs_2['id'])).to_not be_nil
            end
            ...
            it 'records a create event for each dashboard client' do
              ...
              expect(Event.where(type: 'audit.service_dashboard_client.create').count).to eq 2

              event = Event.first(type: 'audit.service_dashboard_client.create', actee_name: client_id)
          ...
          context 'when some, but not all dashboard sso clients exist in UAA' do
            ...
            context 'when the broker has already claimed a requested UAA client' do
              ...
              it 'records a create event for each new dashboard client' do
                ...
                expect(Event.where(type: 'audit.service_dashboard_client.create').count).to eq 1
            ...
            context 'when there is no claim but the requested client exists in UAA' do
              ...
              it 'does not claim any clients for CC' do
                ...
                expect { manager.synchronize_clients_with_catalog(catalog) }.not_to change { ServiceDashboardClient.count }
              end
          ...
          context 'when the UAA has clients claimed by CC that are no longer used by a service' do
            ...
            it 'removes the claims for the deleted clients' do
              manager.synchronize_clients_with_catalog(catalog)
              expect(ServiceDashboardClient.find(uaa_id: unused_id)).to be_nil
            end
            ...
            it 'records a delete event for the deleted client' do
              manager.synchronize_clients_with_catalog(catalog)
              expect(Event.where(type: 'audit.service_dashboard_client.delete').count).to eq 1

              event = Event.first(type: 'audit.service_dashboard_client.delete', actee_name: unused_id)
              expect(event.actor_type).to eq('service_broker')
              expect(event.actor).to eq(service_broker.guid)
              expect(event.actor_name).to eq(service_broker.name)
              expect(event.timestamp).to be
          ...
          context 'when a different broker has already claimed the requested UAA client' do
            ...
            before do
              allow(ServiceDashboardClient).to receive(:find_client_by_uaa_id).with(
        ...
        describe 'exception handling' do
          context 'when getting UAA clients raises an error' do
            before do
              error = UaaError.new('my test error')
          ...
          context 'when modifying UAA clients fails' do
            ...
            before do
              ...
              allow(client_manager).to receive(:modify_transaction).and_raise(UaaError.new('error message'))
            ...
            it 'does not add new claims' do
              ...
              dashboard_client = ServiceDashboardClient.find(uaa_id: dashboard_client_attrs_1['id'])
            ...
            it 'does not delete existing claims' do
              ...
              dashboard_client = ServiceDashboardClient.find(uaa_id: unused_id)
            ...
            it 'does not modify any of the claims' do
              ...
              dashboard_client = ServiceDashboardClient.find(uaa_id: dashboard_client_attrs_2['id'])
            ...
            it 'does not record a create event' do
              manager.synchronize_clients_with_catalog(catalog) rescue nil
              expect(Event.where(type: 'audit.service_dashboard_client.create').count).to eq 0
            end
          end

          context 'when claiming the client for the broker fails' do
            before do
              allow(ServiceDashboardClient).to receive(:claim_client).and_raise
            end
      ...
      describe '#remove_clients_for_broker' do
        ...
        it 'deletes the claims for the service broker in CC' do
          expect(ServiceDashboardClient.find_claimed_client(service_broker).count).to eq(2)

          manager.remove_clients_for_broker

          expect(ServiceDashboardClient.find_claimed_client(service_broker).count).to eq(0)
        end

        it 'records a delete event for each dashboard client' do
          manager.remove_clients_for_broker

          expect(Event.where(type: 'audit.service_dashboard_client.delete').count).to eq 2

          event = Event.first(type: 'audit.service_dashboard_client.delete', actee_name: client_to_delete_1)
          expect(event.actor_type).to eq('service_broker')
          expect(event.actor).to eq(service_broker.guid)
          expect(event.actor_name).to eq(service_broker.name)
          expect(event.timestamp).to be
        ...
        context 'when deleting UAA clients fails' do
          before do
            error = UaaError.new('error message')
          ...
          it 'does not record any events' do
            manager.remove_clients_for_broker rescue nil
            expect(Event.where(type: 'audit.service_dashboard_client.delete').count).to eq 0
          end
        end
        ...
        context 'when removing CC claims raises an exception' do
          before do
            allow(ServiceDashboardClient).to receive(:release_client).and_raise('test error')
          end
    ...
    context 'for service instances' do
      ...
      let(:dashboard_client) { ServiceInstanceDashboardClient }
