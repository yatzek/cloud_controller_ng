...
module CloudController
  module Blobstore
    describe BitsServiceClient do
      ...
      subject(:client) { BitsServiceClient.new(bits_service_options: options, resource_type: resource_type) }
      ...
      describe '#exists?' do
        ...
        context 'when the response code is invalid' do
          ...
          it 'raises a BlobstoreError' do
            expect { subject.exists?(key) }.to raise_error(BlobstoreError)
          end
        end
      end
      ...
      describe '#cp_to_blobstore' do
        ...
        context 'when the response code is not 201' do
          it 'raises a BlobstoreError' do
            ...
            expect { subject.cp_to_blobstore(file_path, key) }.to raise_error(BlobstoreError)
          end
        end
      end
