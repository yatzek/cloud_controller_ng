...
module VCAP::CloudController::ResourcePool
  describe LegacyPool do
    include_context 'resource pool'

    subject(:resource_pool) { LegacyPool.new(resource_pool: config) }
    ...
    describe '#copy' do
      ...
      before do
        ...
        allow(resource_pool).to receive(:blobstore).and_return(blobstore)
      end

      it 'streams the resource to the destination' do
        expect(blobstore).to receive(:download_from_blobstore).with(
          descriptor['sha1'],
          'some/destination',
