...
describe Runner do
  ...
  let(:diagnostics) { instance_double(Diagnostics) }
  let(:periodic_updater) { instance_double(Metrics::PeriodicUpdater) }
  let(:routing_api_client) { instance_double(RoutingApi::Client, router_group_guid: '') }
  ...
  before do
    ...
    allow(Metrics::PeriodicUpdater).to receive(:new).and_return(periodic_updater)
    allow(periodic_updater).to receive(:setup_updates)
    allow(VCAP::PidFile).to receive(:new) { double(:pidfile, unlink_at_exit: nil) }
    allow(Diagnostics).to receive(:new).and_return(diagnostics)
