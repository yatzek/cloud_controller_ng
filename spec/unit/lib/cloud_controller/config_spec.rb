...
describe Config do
  ...
  describe '.configure_components' do
    ...
    it 'creates the runners' do
      expect(Runners).to receive(:new).with(
        @test_config,
        message_bus,
    ...
    it 'creates the stagers' do
      expect(Stagers).to receive(:new).with(
        @test_config,
        message_bus,
        instance_of(Dea::Pool))
      Config.configure_components(@test_config)
    ...
    it 'creates the nsync client' do
      ...
      Config.configure_components(@test_config)
      expect(dependency_locator.nsync_client).to be_an_instance_of(::Diego::NsyncClient)
    end
    ...
    it 'creates the stager client' do
      ...
      Config.configure_components(@test_config)
      expect(dependency_locator.stager_client).to be_an_instance_of(::Diego::StagerClient)
    end
