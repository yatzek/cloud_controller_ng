...
module VCAP::CloudController
  RSpec.describe V2::LifecycleCreate do
    ...
    let(:app) { AppModel.create(name: 'supscool') }
    subject(:lifecycle_create) { described_class.new('user', 'user_email') }
    ...
    describe 'create' do
      context 'when both a buildpack and stack guid are requested' do
        ...
        it 'creates a buildpack lifecycle with a stack on the app' do
          ...
          buildpack_lifecycle = BuildpackLifecycleDataModel.last

          expect(buildpack_lifecycle.app.name).to eq('supscool')
          expect(buildpack_lifecycle.buildpack).to eq('http://example.com/buildpack')
          expect(buildpack_lifecycle.stack).to eq('stacks-on-stacks')
        end
      end
      ...
      context 'when only buildpack is requested' do
        ...
        it 'creates a buildpack lifecycle with a default stack' do
          ...
          expect(buildpack_lifecycle.app.name).to eq('supscool')
          expect(buildpack_lifecycle.buildpack).to eq('http://example.com/buildpack')
          expect(buildpack_lifecycle.stack).to eq(Stack.default.name)
        end
      end
