  ...
  describe '#delete' do
    ...
    describe 'recursive deletion' do
      ...
      context 'when private brokers are associated with the space' do
        let!(:service_to_be_deleted)      { Service.make(service_broker: broker_to_be_deleted) }
        let!(:service_plan_to_be_deleted) { ServicePlan.make(service: service_to_be_deleted) }
        let!(:broker_to_be_deleted)       { ServiceBroker.make(space_guid: space.guid) }
        let!(:broker_to_be_deleted2) { ServiceBroker.make(space_guid: space.guid) }
