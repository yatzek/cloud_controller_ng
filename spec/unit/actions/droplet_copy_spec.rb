...
describe DropletCopy do
  ...
  let(:target_app) { AppModel.make(name: 'target-app-name') }
  let(:source_app) { AppModel.make(name: 'source-app-name', space_guid: source_space.guid) }
  let(:lifecycle_type) { :buildpack }
  let!(:source_droplet) { DropletModel.make(lifecycle_type,
