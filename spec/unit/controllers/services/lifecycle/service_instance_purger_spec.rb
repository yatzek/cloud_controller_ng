...
describe CloudController::ServiceInstancePurger do
  let(:event_repository) { Repositories::ServiceEventRepository.new({ user: User.make, user_email: 'email' }) }
  let(:purger) { CloudController::ServiceInstancePurger.new(event_repository) }
