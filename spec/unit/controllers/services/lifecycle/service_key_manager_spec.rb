...
describe CloudController::ServiceKeyManager do
  ...
  context '#delete' do
    ...
    it 'use the delete action to delete the key' do
      expect(ServiceKeyDelete).to receive(:new).and_return(service_key_delete_action)
      expect(Jobs::DeleteActionJob).to receive(:new).with(ServiceKey, service_key.guid, service_key_delete_action).and_return(delete_action_job)
