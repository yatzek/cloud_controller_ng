...
describe CloudController::AppsController do
  ...
  describe 'create app' do
    ...
    describe 'events' do
      it 'records app create' do
        ...
        expected_attrs = CloudController::AppsController::CreateMessage.decode(initial_hash.to_json).extract(stringify_keys: true)
    ...
    context 'when allow_ssh is enabled globally' do
      before do
        ...
        allow(Config.config).to receive(:[]).with(:allow_app_ssh_access).and_return true
      end
    ...
    context 'when allow_ssh is disabled globally' do
      before do
        set_current_user_as_admin
        allow(Config.config).to receive(:[]).with(anything).and_call_original
        allow(Config.config).to receive(:[]).with(:allow_app_ssh_access).and_return false
      end
  ...
  describe 'on route change' do
    ...
    it 'tells the dea client to update when we add one url through PUT /v2/apps/:guid' do
      ...
      expect(Dea::Client).to receive(:update_uris).with(an_instance_of(App)) do |app|
