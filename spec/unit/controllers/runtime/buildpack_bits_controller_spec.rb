...
describe CloudController::BuildpackBitsController do
  let(:user) { make_user }
  ...
  context 'Buildpack binaries' do
    let(:test_buildpack) { Buildpack.create_from_hash({ name: 'upload_binary_buildpack', position: 0 }) }
    ...
    context '/v2/buildpacks/:guid/bits' do
      ...
      it 'does not allow upload if the buildpack is locked' do
        locked_buildpack = Buildpack.create_from_hash({ name: 'locked_buildpack', locked: true, position: 0 })
      ...
      context 'when the same bits are uploaded twice' do
        let(:test_buildpack2) { Buildpack.create_from_hash({ name: 'buildpack2', position: 0 }) }
        before do
          put "/v2/buildpacks/#{test_buildpack.guid}/bits", { buildpack: valid_zip2 }
    ...
    context '/v2/buildpacks/:guid/download' do
      ...
      before do
        ...
        Buildpack.create_from_hash({ name: 'get_binary_buildpack', key: 'xyz', position: 0 })
      end
