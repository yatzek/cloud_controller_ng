RSpec::Matchers.define :be_a_fully_wrapped_job_of do |expected_job_class|
  match do |actual_job|
    ...
    return false unless next_job.is_a? Jobs::ExceptionCatchingJob

    next_job = next_job.handler
    return false unless next_job.is_a? Jobs::RequestJob

    next_job = next_job.job
    return false unless next_job.is_a? Jobs::TimeoutJob
